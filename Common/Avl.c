...
*/
static int
avl_min( const void * elt1, const void * elt2, AvlNodeType nd_typ )
{
...
*/
static int
avl_max( const void * elt1, const void * elt2, AvlNodeType nd_typ )
{
  if ( nd_typ == AVL_IS_RBRANCH  ||  nd_typ == AVL_IS_LEAF )
  ...
  )
{
  ...
  if ( cmp < 0 )   /* insert into the left subtree */
    ...
    if ( cmp > 0 )   /* insert into the right subtree */
      {
	increase =  avl_insert( avlDesc,
				data,
				&( (*rootp) -> subtree[ RIGHT ] ),
  ...
  )
{
  ...
  if ( cmp < 0 )   /* delete from left subtree */
    ...
    if ( cmp > 0 )   /* delete from right subtree */
      {
	decrease =  avl_delete( avlDesc,
				data,
				&( (*rootp) -> subtree[ RIGHT ] ),
...
*	are declared in the include file "Common.h"
*
***********************************************************************/
...
void *
AvlSetTreeData( AvlTree tree, void * data )
{
  AVLdescriptor * avl_desc = (AVLdescriptor *) tree;
  void * old = avl_desc->root->treeData;
  avl_desc->root->treeData = data;
  return( old );
}

void *
AvlGetTreeData( AvlTree tree )
{
  AVLdescriptor  *avl_desc = (AVLdescriptor *) tree;
